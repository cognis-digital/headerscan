...
**headerscan** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `headerscan` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
headerscan ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
headerscan ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
headerscan ... --format json | cognis-connect emit --to sigma   # Sigma rules
headerscan ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
```
...
works whatever `headerscan` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="headerscan") for rec in records]   # records = your JSON output
