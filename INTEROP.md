...
How **headerscan** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `headerscan` lives in the **Threat intel / detections** cluster.
...
| from | relation | to |
|---|---|---|
| `headerscan` | JSON findings compose with | [`stixgen`](https://github.com/cognis-digital/stixgen), [`iocextract`](https://github.com/cognis-digital/iocextract), [`attackmap`](https://github.com/cognis-digital/attackmap), [`ttphunt`](https://github.com/cognis-digital/ttphunt) |
| `headerscan` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `headerscan` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `headerscan` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the threat intel / detections cluster.** `headerscan` output feeds the next tool:
```bash
headerscan ... --format json > out.json          # this tool's findings (see `headerscan --help`)
...
headerscan ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
headerscan ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
headerscan ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `headerscan`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `headerscan` slots
