...
- **rules/analyzers** apply the heuristics shipped in `headerscan/core.py`.
- **score** ranks by severity.
- **MCP server** (`headerscan mcp`) exposes `scan` for Cognis.Studio agents.
