"""Native cognis-connect emit for headerscan — forward findings to any platform.

Maps headerscan's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    headerscan ... --format json | headerscan-emit --to stix
    headerscan-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('header') or rec.get('finding')
        out['severity'] = 'info' if rec.get('level') == 'info' else 'low'
        out['type'] = rec.get('type') or 'header'
        out['description'] = rec.get('message')
        out['tags'] = [rec.get('header'), rec.get('type')]
